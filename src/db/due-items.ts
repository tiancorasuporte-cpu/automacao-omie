import { getDb, type DueItem, type DueItemType } from "./schema";
...
export async function listDueItems(filters?: {
  ...
  const statusFilter = openOnly
    ? db`and coalesce(status, '') not in ('RECEBIDO', 'CANCELADO', 'BAIXADO', 'LIQUIDADO', 'PAGO')`
    : db``;
...
export async function getDueItemsForNotification(targetDate: string) {
  ...
  const rows = await db<DueItemRow[]>`
    ...
    where due_date = ${targetDate}::date
      ...
      and coalesce(status, '') not in ('RECEBIDO', 'CANCELADO', 'BAIXADO')
      and not exists (
        select 1
...
export async function insertNotificationLog(input: {
  ...
  const db = await getDb();
  await db`
    insert into notification_log (due_item_id, phone, message, success, error)
    values (${input.dueItemId}, ${input.phone}, ${input.message}, ${input.success}, ${input.error ?? null})
  `;
}
...
export async function getDashboardStats() {
  ...
  >`
    select
      ...
      count(*) filter (where due_date = current_date + 1)::int as due_tomorrow,
      count(*) filter (
        where due_date between current_date and current_date + 7
          and due_date <= (date_trunc('month', current_date) + interval '1 month' - interval '1 day')::date
      )::int as due_week,
      count(*) filter (where due_date = current_date + 1 and notified_at is null)::int as pending_notifications,
      count(*) filter (
        where coalesce(status, '') not in ('RECEBIDO', 'CANCELADO', 'BAIXADO')
          and due_date >= date_trunc('month', current_date)::date
          and due_date <= (date_trunc('month', current_date) + interval '1 month' - interval '1 day')::date
      )::int as total_open
    from due_items
