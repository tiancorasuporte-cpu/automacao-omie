...
export const getDueItemBoletoViewFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!document?.url) {
      return { ok: false as const, error: "PDF do boleto indisponível no Omie." };
    }

    return { ok: true as const, url: document.url, filename: document.filename };
  });
...
export const getNotificationSettingsFn = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const hourlyParsed = Number(settings.notification_hourly_limit ?? "20");
  return {
    ...
    hourlyLimit: Number.isFinite(hourlyParsed) && hourlyParsed > 0 ? Math.min(hourlyParsed, 200) : 20,
  };
});
